...
class KS4EMUsortSorter(BaseSorter):
    ...
    _params_description = {
        ...
        "hdbscan_min_cluster_size": "Setting added for EMUsort to control the minimum number of points needed to designate as a cluster during HDBSCAN spike outlier removal. Default value: 20",
    ...
    def _run_from_folder(cls, sorter_output_folder, params, verbose):
        ...
        try:  # if version.parse(cls.get_sorter_version()) >= version.parse("4.0.12"):
            ...
            ops = initialize_ops(
                settings, probe, recording.get_dtype(), do_CAR, invert_sign, device
            )
            (
                n_chan_bin,
                fs,
