...
def extract_snippets(
    ...
    if isinstance(Th_single_ch, (int, float)):
        Th_list = []
        ...
        raise ValueError("Th_single_ch must be either int, float, or iterable")
    xy_all = None
    for iTh in Th_list:
        Xabs = X.abs()
...
def extract_wPCA_wTEMP(
    ...
    if (
        ops["settings"]["remove_spike_outliers"]
        and clips.shape[0] >= min_number_of_spikes
    ):
        hdbscan = HDBSCAN(
            min_cluster_size=ops["settings"]["hdbscan_min_cluster_size"]
        ).fit_predict(clips)
        ...
        if (
            clips.shape[0] < min_number_of_spikes
            and ops["settings"]["remove_spike_outliers"]
        ):
            print(
                f"Skipping spike outlier removal with HDBSCAN because there must be at least {min_number_of_spikes} spikes identified, but there were {clips.shape[0]}. To use outlier removal, try a longer recording or lower Th_single_ch."
            )
        bad_clips = None
        good_clips = clips
    ...
    ## KMeans clustering
    km_model = KMeans(n_clusters=ops["settings"]["n_templates"], n_init=10).fit(
        good_clips
    )
