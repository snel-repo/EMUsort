...
def prepare_extract(ops, U, nC, device=torch.device("cuda")):
    ds = (ops["xc"] - ops["xc"][:, np.newaxis]) ** 2 + (
        ops["yc"] - ops["yc"][:, np.newaxis]
    ) ** 2
...
def extract(ops, bfile, U, device=torch.device("cuda"), progress_bar=None):
    ...
    for ibatch in tqdm(
        ...
        stt = stt.double()
        st[k : k + nsp, 0] = (
            ((stt[:, 0] - nt) + ibatch * (ops["batch_size"])).cpu().numpy()
            - nt // 2
            + ops["nt0min"]
        )
...
def postprocess_templates(Wall, ops, clu, st, device=torch.device("cuda")):
    ...
    Wall3, _, _ = merging_function(
        ops, Wall2.transpose(1, 2), clu, st[:, 0], 0.9, "mu", device=device
    )
...
def run_matching(ops, X, U, ctc, device=torch.device("cuda")):
    ...
    for t in range(100):
        ...
        n = 2
        for j in range(n):
            Xres[:, iX[j::n] + tiwave] -= amp[j::n] * torch.einsum(
                "ijk, jl -> kil", U[iY[j::n, 0]], W
            )
...
def merging_function(
    ops, Wall, clu, st, r_thresh=0.5, mode="ccg", device=torch.device("cuda")
):
    clu2 = clu.copy()
    ...
    while t < NN:
        ...
        mu = (Ww**2).sum((1, 2), keepdims=True) ** 0.5
        Wnorm = Ww / (1e-6 + mu)
        ...
        for j in range(NN):
            ...
            if mode == "ccg":
                ...
                _, is_ccg, _ = CCG.check_CCG(
                    st0, st1, acg_threshold=acg_threshold, ccg_threshold=ccg_threshold
                )
            else:
