...
class KS4EMUsortSorter(BaseSorter):
    """Kilosort4 Sorter object, modified for use with EMUsort."""

    sorter_name: str = "ks4-emusort"
    ...
    def _dynamic_params(cls):
        if cls.is_installed():
            ...
            warnings.warn(
                "Kilosort4 is not installed. Please install kilosort4 to get the parameters."
            )
            return {}, {}
    ...
    def initialize_folder(
        ...
        return super().initialize_folder(
    ...
    def _run_from_folder(cls, sorter_output_folder, params, verbose):
        ...
        setup_logger_takes_verbose_console = version.parse(
            cls.get_sorter_version()
        ) > version.parse("4.0.18")
        logger_is_named = version.parse(cls.get_sorter_version()) > version.parse(
            "4.0.20"
        )
        ...
        if not params["skip_kilosort_preprocessing"]:
            ops = compute_preprocessing(
                ops=ops, device=device, tic0=tic0, file_object=file_object
            )
        else:
            if verbose:
        ...
        if params["skip_kilosort_preprocessing"]:
            ops["preprocessing"] = dict(
                hp_filter=torch.as_tensor(np.zeros(1)),
                whiten_mat=torch.as_tensor(np.eye(recording.get_num_channels())),
            )
