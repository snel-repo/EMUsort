...
# get local module
from ..ks4mods.preprocessing import fft_highpass, get_drift_matrix
...
def find_binary(data_dir: Union[str, os.PathLike]) -> Path:
    ...
    filenames = (
        list(data_dir.glob("*.bin"))
        + list(data_dir.glob("*.bat"))
        + list(data_dir.glob("*.dat"))
        + list(data_dir.glob("*.raw"))
    )
    if len(filenames) == 0:
    ...
    if len(filenames) > 1:
        raise ValueError(
            'Multiple binary files in folder with "ap" tag, please specify filename'
        )
...
def load_probe(probe_path):
    ...
    if probe_path.suffix == ".prb":
        ...
        probe["n_chan"] = (
            (mat["chanMap"] - 1).ravel().astype(np.int32).shape[0]
        )  # NOTE: should match the # of columns in the raw data
...
def save_to_phy(
    ...
    templates_ind = np.tile(
        np.arange(Wall.shape[1])[np.newaxis, :], (templates.shape[0], 1)
    )
    ...
    for stype, prop in zip(stypes, props):
        ...
        if stype == "KSLabel":
            shutil.copyfile(
                (results_dir / f"cluster_{stype}.tsv"),
...
def save_ops(ops, results_dir=None):
    ...
    ops["preprocessing"] = {k: v.cpu().numpy() for k, v in ops["preprocessing"].items()}
...
def load_ops(ops_path, device=None):
    ...
    #       at different levels?
    ops["preprocessing"] = {
        k: torch.from_numpy(v).to(device) for k, v in ops["preprocessing"].items()
    }
...
class BinaryRWFile:
    ...
    def padded_batch_to_torch(self, ibatch, return_inds=False):
        ...
        with warnings.catch_warnings():
            ...
            if ibatch == 0:
                X[:, self.nt : self.nt + nsamp] = (
                    torch.from_numpy(data).to(self.device).float()
                )
...
class BinaryFileGroup:
    ...
    def from_filenames(filenames, n_channels, dtype=np.int16, mode="r"):
        ...
        for name in filenames:
            ...
            f = np.memmap(name, mode=mode, dtype=dtype, shape=(n_samples, n_channels))
            files.append(f)
...
class BinaryFiltered(BinaryRWFile):
    def __init__(
        ...
        nt: int = 61,
        nt0min: int = 20,
        chan_map: np.ndarray = None,
        hp_filter: torch.Tensor = None,
        whiten_mat: torch.Tensor = None,
        chan_delays: torch.Tensor = None,
        dshift: torch.Tensor = None,
        device: torch.device = None,
        do_CAR: bool = True,
        artifact_threshold: float = np.inf,
...
class RecordingExtractorAsArray:
    def __init__(self, recording_extractor):
        ...
        self.c = self.recording.get_traces(
            start_frame=0, end_frame=1, segment_index=0
        ).shape[1]
