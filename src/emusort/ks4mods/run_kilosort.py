...
def set_files(
    ...
    results_dir = (
        settings.get("results_dir", None) if results_dir is None else results_dir
    )
    ...
    if probe is None:
        ...
        elif "probe_name" in settings:
            probe_path = PROBE_DIR / settings["probe_name"]
...
def initialize_ops(
    ...
    if not settings["templates_from_data"] and settings["nt"] != 61:
        raise ValueError(
            "If using pre-computed universal templates "
...
def compute_preprocessing(ops, device, tic0=np.nan, file_object=None):
    ...
    if ops["remove_chan_delays"]:
        chan_delays, best_chan = preprocessing.get_channel_delays(
            bfile, fs=fs, nskip=nskip, device=device
        )
    else:
    ...
    logger.info(
        f"Preprocessing filters computed in {time.time() - tic: .2f}s; "
...
def detect_spikes(
    ...
    logger.info(
        f"{len(st)} spikes extracted in {time.time() - tic: .2f}s; "
        + f"total {time.time() - tic0: .2f}s"
    )
    logger.debug(f"st shape: {st.shape}")
    logger.debug(f"tF shape: {tF.shape}")
...
def cluster_spikes(
    ...
    logger.info("-" * 40)
    Wall, clu, is_ref = template_matching.merging_function(
        ops, Wall, clu, st[:, 0], device=device
    )
    clu = clu.astype("int32")
    logger.info(
        f"{clu.max() + 1} units found, in {time.time() - tic: .2f}s; "
        + f"total {time.time() - tic0: .2f}s"
    )
    logger.debug(f"clu shape: {clu.shape}")
    logger.debug(f"Wall shape: {Wall.shape}")
...
def save_sorting(
    ...
    logger.info(
        f"Total runtime: {runtime:.2f}s = {int(hrs):02d}:"
