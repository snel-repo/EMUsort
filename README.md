...
4. The number of channels we have tested ranges mostly from 4-16. If the channel count is outside this range, it is likely that certain channel and template size-related parameters would need to be tuned for best performance, such as `min_template_size`, `template_sizes`, `nearest_chans`, and `nearest_templates`, where `nearest_templates` can be set equal to the number of channels for better numerical stability. Currently, sorting of 1 channel is not supported due to the sorting improvement seen in template matching when signals from the same MUAP appear on multiple channels.
...
The `linked_params_for_sweep` parameter controls how the sweep combinations are explored by using only matched indexes for linked parameters. If no linked parameters are specified (e.g., if `linked_params_for_sweep` is left blank), the Kilosort parameter combinations will be explored in full, so that the product of the number of elements in each Kilosort parameter list defined in `KS_params_to_sweep` is the total number of combinations. In this case, beware of the combinatorics so you don't generate more sorts than you expected (e.g., NxM combinations for N of param1, M of param2, etc.). With `linked_params_for_sweep`, you can specify lists of linked parameters where each element is a list of parameter keys, such as `linked_params_for_sweep: [[Th_universal, Th_learned]]` for a single linkage, or `linked_params_for_sweep: [[Th_universal, Th_learned], [nt, nt0min]]` for two different linkages. When parameters are linked, the first element of each parameter will be used with the first element of all other parameters in the group, the second elements of linked parameters are used together, and so on. This means each Kilosort parameter list for linked parameters must be equal length. If you want, you may also link more than 2 parameters together, such as `linked_params_for_sweep: [[Th_universal, Th_learned, Th_single_ch]]`.

By using `linked_params_for_sweep`, you get explicit control of some Kilosort parameter combinations to avoid bad combinations and reduce the overall number of runs to be performed. To determine the total number of combinations for a given sweep when using `linked_params_for_sweep`, you can treat the linked parameters as a single parameter in the combinatorics multiplication.
